...
import OtherUserContent from '../components/OtherUserContent.jsx'
...
function Profile() {
    ...
    const selectedUserData = async() => {
        try {
            const response = await axios.get(backendURL + '/api/user/get/other', {params: { userId }});
            if(response.data.success)
                setSelectedUser(response.data.userData);
        } 
        catch (error) {
            toast.error(error.message);    
        }
    ...
    if(Object.keys(selectedUser).length > 0 && userId !== userData._id) {
        return (
            <div>
                <div>
                    {/* profile banner */}
                        ...
                        <div className='md:w-1/2 lg:w-1/3'>
                            <div className='w-full flex items-center justify-center mb-5'>
                                <div className='h-35 w-35 sm:h-25 sm:w-25'> 
                                    <Avatar src={ selectedUser.profile } /> 
                                </div>
                            </div>

                            <div className='flex flex-col w-full items-center justify-center mb-5 md:mb-0'> 
                                <p className='text-(--contrast-colour) font-bold text-2xl text-center'> { selectedUser.username } </p>
                            </div>
                        </div>
                    ...
                    <div className='bg-(--post-background) shadow rounded-b-lg mx-6 sm:mx-16 xl:mx-40 p-5 mb-10'>
                        <div className='mt-4 mx-4'>
                            ...
                            {
                                <OtherUserContent profileData={selectedUser.createdBlogs} />
                            }
                        </div>
                    </div>
                </div>
